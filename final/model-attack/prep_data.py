...
DATA_PATH = "D:/Study/AttackTransferLearning/data"
RANDOM_STATE = 1234

def pad_transform(x):
    """Pads the image tensor."""
    return F.pad(x, (2, 2, 2, 2), 'constant', 0)

def expand_transform(x):
...
def get_dataset(dataname, data_split='train', use_normalize=False, test_size=0.1, train_size=None):
    ...
    if dataname == "mnist":
        ...
        transform_ops += [transforms.Lambda(pad_transform), 
                  transforms.Lambda(expand_transform)]
        if use_normalize:
